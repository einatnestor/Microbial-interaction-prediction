...
CARBONS = ['ArabinoseD', 'ArabinoseL', 'Xylose',
           'Ribose', 'Rhamnose', 'Fructose', 'Galactose', 'Glucose', 'Glucose01', 'Mannose', 'GlcNAc', 'Acetate',
           'Pyruvate',
           'Pyruvate01', 'Fumarate', 'Succinate', 'Citrate', 'Glycerol', 'Glycerol01', 'Mannitol', 'Sorbitol',
           'Alanine',
           'Serine', 'Proline', 'Proline01', 'Glutamine', 'Arginine', 'Trehalose', 'Cellobiose', 'Maltose', 'Sucrose',
...
def readPhylogeneticMatrix():
    ...
    """
    table = pd.read_csv("Data_msystems/pairwiseDistances-21Isolates&Ecoli.csv")
...
def calcDistance(type, ith_closest):
    ...
    """
    distance_rr, closest = [], []
    # distance for carbon - Euclidean distnace from Metabolic pca, distnace for strain - fro phylogenetic distance matrix.
    if type == "carbon":
...
def runAllStrainsMetrics(X, y, modelType, mono):
    ...
    :param mono: Boolean for mono features
    :return: Metrics df
    """
    if modelType == 'sign':
...
def createArray(allOtherCarbons):
    """
    Create array of the input carbons
    """
    otherArr = []
...
def createPcaForCarbron(dimensions, carbon, new_carbon, allOtherCarbons):
    ...
    """
    carbon_table = pd.read_csv("Data_msystems/metabolic_profiles.csv").drop(['carbon'], axis=1)
    coulmns_names = carbon_table.columns
    # carbon_table = StandardScaler().fit_transform(carbon_table)
    carbon_table = pd.DataFrame(carbon_table, columns=coulmns_names)
    withoutNew = carbon_table.iloc[allOtherCarbons, 0:]
    newCarbon = carbon_table.iloc[new_carbon, 0:]
    pca = PCA()
...
def FullyTrainedStrengthStrain(X_train, y_train, X_test, y_test):
    ...
    """
    model = xgboost.XGBRegressor(base_score=0.6, booster='gbtree', colsample_bylevel=1,
                 colsample_bynode=1, colsample_bytree=1, enable_categorical=False,
...
def runClassification(X, y, mono):
    ...
    :param y: labels
    :param mono: bool for addition of monoculture growth yield features
    :return: metrics df
    """
    ...
    for strain in STRAINS:
        df_copy =  copy.copy(X)
        if not mono:
            df_copy = df_copy.drop(columns=['monoGrow_x', 'monoGrow_y', 'monoGrow24_x', 'monoGrow24_y', 'metDis'], axis=1)
        ...
        X_train_without_strain, X_test_without_strain, y_train_without_strain, y_test_without_strain = createTeststrain(df_copy, y, strain)
        ...
        scores = metricPerStrain(X_train_without_strain_x_rf,y_train_without_strain,X_test_without_strain_x_rf, y_test_without_strain)
...
def runRegression(X, y, mono):
    ...
    :param X: Full features table
    :param y: Labels
    :param mono: Boolean for addition of monoculture growth yield features
    :return: Metrics df
    """
    distance_matrix_strains = readPhylogeneticMatrix()
    ...
    for strain in STRAINS:
        ...
        if not mono:
            ...
            pca_carbon_no_strain = createPCA("carbon", 4)
        df_copy = pd.merge(df_copy, pca_carbon_no_strain, how='left', right_on=['carbon'],left_on=['Carbon']).drop(columns=['carbon'], axis=1)
...
def runClassificationCarbon(X, y, mono):
    ...
    for carbon in CARBONS:
        ...
        df_copy = pd.merge(df_copy, pca_carbon, how='left', right_on=['carbon'], left_on=['Carbon']).drop( columns=['carbon'])
        if not mono:
            df_copy = df_copy.drop(columns=['monoGrow_x', 'monoGrow_y', 'monoGrow24_x', 'monoGrow24_y', 'metDis'],axis=1)
...
def signNaiveIthClosest(X, y):
    ...
    #allDistancesdf.to_csv("Data_msystems/Naive_copy_sign_per_distance.csv")


def strengthNaiveIthClosest(X, y):
    """
    ...
    for strain in STRAINS:
        ...
        for i in range(len(STRAINS) - 1):
            ...
            new_line = {'r2': scores_naive['R2'], 'rmse': scores_naive['RMSE'], 'strain': strain,
                        ...
                        'distance': ith_distance[0], 'ith': i}
            allDistancesdf = allDistancesdf.append(new_line, ignore_index=True)
    #allDistancesdf.to_csv("Data_msystems/Naive_copy_strength_per_distance.csv")
...
def saveSign(type, index_df, metrics_per_strain_mono, metrics_per_strain_no_mono, full_metrics):
    ...
    #pairplot_strain.to_csv("Data_msystems/Partial_models_sign_" + type + ".csv")
    with open("Data_msystems/full_metrics_" + type + "_classification.csv", 'w') as csvfile:
...
def saveStrength(type, index_df, metrics_per_strain_mono, metrics_per_strain_no_mono, full_metrics):
    ...
    #pairplot_strain.to_csv("Data_msystems/Partial_models_strength_" + type + ".csv")
    with open("Data_msystems/full_metrics_" + type + "_regression.csv", "w") as csvfile:
...
def testUknown(X, y, untrainedType, modelType):
    ...
    :param X: Features
    :param y: y labels
    :param untrainedType: strain/ carbon
    :param modelType: sign/ strength
    :return:
    """
    # sign prediction
    if modelType == 'sign':
        ...
        if untrainedType == 'strain':
            ...
            saveSign(untrainedType, strain_df, metrics_per_strain_mono, metrics_per_strain_no_mono,full_metrics)
            ...
            saveSign(untrainedType, carbon_df, metrics_per_carbon_mono, metrics_per_carbon_no_mono,
                               full_metrics)
    # strength prediction
    else:
        ...
        if untrainedType == 'strain':
            strengthNaiveIthClosest(X, y)
            distance_info_strain = calcDistance("strain", 0)
            metrics_per_strain_mono = runAllStrainsMetrics(X, y, modelType, True)
            metrics_per_strain_no_mono = runAllStrainsMetrics(X, y, modelType, False)
            saveStrength(untrainedType, strain_df, metrics_per_strain_mono, metrics_per_strain_no_mono, full_metrics)

        elif untrainedType == 'carbon':
            distance_info_carbon = calcDistance("carbon", 0)
            metrics_per_carbon_mono = runAllCarbonsMetrics(X, y, modelType, True)
            metrics_per_carbon_no_mono = runAllCarbonsMetrics(X, y, modelType, False)
            saveStrength(untrainedType, carbon_df, metrics_per_carbon_mono, metrics_per_carbon_no_mono,full_metrics)
...
def fullyTraindMetricsSign(X_test):
    """
    Calculate all metrics for the full model (strength prediction)
    :param X_test: A df contains all needed information
    :return: Strain df, carbon df
    """
...
def signMetricasGroupByStrain():
    ...
    X, y = features[['Carbon', 'Bug 1','monoGrow_x', 'monoGrow_y', 'monoGrow24_x', 'monoGrow24_y', 'metDis', 'carbon_component_0',
    ...
    X_train, X_test = X_train.drop(['Carbon', 'Bug 1'], axis=1), X_test.drop(['Carbon', 'Bug 1'],axis=1)
    y_test = create_binary(y_test)
    y_train = create_binary(y_train)

    model = pickle.load(open('models/sign_models/sign_model.sav', 'rb'))
    y_pred = model.predict(X_test)
    X_test['y'], X_test['y_pred'], X_test['strain_x'], X_test['Carbon'] = y_test, y_pred, X_test_info['Bug 1'],  X_test_info['Carbon']
    X_test = X_test[["Carbon", "strain_x", "y", "y_pred"]]
    X_test['neg_pred'] = np.full(X_test.shape[0], -1)
    # Scores grouped by strain or carbon
...
def strengthMetricasGroupByStrain():
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=10)
    X_test_info = X_test[['Bug 1', 'Carbon']]
    X_train, X_test = X_train.drop(['Carbon', 'Bug 1'], axis=1), X_test.drop(
        ['Carbon', 'Bug 1'],
        axis=1)

    model = pickle.load(open('models/strength_models/strength_model.sav', 'rb'))
...
def main():
    df = pd.read_csv("Data_msystems/Features.csv").drop(['Unnamed: 0'], axis=1)
    ...
    # testUknown(X_r, y_r, 'strain', 'strength')
    # testUknown(X_r, y_r, 'carbon', 'strength')
    # testUknown(X, y, 'strain', 'sign')
    # testUknown(X, y, 'carbon', 'sign')
