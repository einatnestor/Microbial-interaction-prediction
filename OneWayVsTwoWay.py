...
def compare_two_vs_one_binary(one_way_train, y_train_one_way, one_way_test, y_test_one_way, X_train, X_test,
    ...
    one_way_model = xgboost.XGBClassifier(base_score=0.5, booster='gbtree',
                                          colsample_bynode=1,
                                          eta=0.3, gamma=0.001,
                                          ...
                                          scale_pos_weight=1, subsample=1, tree_method='exact').fit(
    ...
    two_way_model = RandomForestClassifier(bootstrap=False, min_samples_split=20, n_estimators=61)
...
def preformanceWithReciprocal(X_train, X_test, y_train_effects, y_test_effects):
    ...
    """
    model_2_1_effect = xgboost.XGBRegressor(base_score=0.2, booster='gbtree', colsample_bylevel=0.4,
    ...
    model_1_2_effect = xgboost.XGBRegressor(base_score=0.2, booster='gbtree', colsample_bylevel=0.4,
                                            colsample_bynode=1, colsample_bytree=1,
                                            eta=0.3, gamma=0.01,
                                            learning_rate=0.1, max_delta_step=2,
                                            max_depth=9, min_child_weight=11,
...
def compareTwoVsOneEffect(one_way_train, y_train_one_way, one_way_test, y_test_one_way, X_train, X_test,
    ...
    :return: Average of nrsme and r2 for each type of model.
    """
    one_way_model = xgboost.XGBRegressor(base_score=0.2, booster='gbtree', colsample_bylevel=0.4,
                                         colsample_bynode=1, colsample_bytree=1,
                                         eta=0.3, gamma=0.01,
                                         learning_rate=0.1, max_delta_step=2,
                                         max_depth=9, min_child_weight=11,
                                         ...
                                         num_parallel_tree=1, predictor='auto', random_state=0,
                                         reg_alpha=0.1, reg_lambda=0.2222222222222222, scale_pos_weight=1,
                                         subsample=0.8, tree_method='exact').fit(
        one_way_train.drop(['Bug 1', 'Bug 2', 'Carbon'], axis=1), y_train_one_way)
    ...
    two_way_model = RandomForestRegressor(bootstrap=False, max_features='sqrt', min_samples_split=4, n_estimators=378)
    two_way_model.fit(X_train, y_train_effects)
    ...
    return rmse_2_1, avg_one_nrmse, avg_one_r2, avg_multi_nrmse, avg_multi_r2
...
def create_one_way_data(X_train, X_test, y_train_effect_2_1, y_test_effect_2_1):
    ...
    full_one_way = pd.read_csv("./Data/Features.csv").drop(['Unnamed: 0'], axis=1)
    ...
    one_way_train = pd.concat([X_train.drop(['1 on 2: Effect'], axis=1), copy])
...
def main():
    no_duplicate = pd.read_csv("./Data/no_duplicate.csv")
    ...
    one_way_train, y_train_one_way, one_way_test, y_test_one_way = create_one_way_data(X_train, X_test,
                                                                                       y_train_effect_2_1,
                                                                                       y_test_effect_2_1)
    with_duplicates = pd.read_csv("./Data/Features_two_way.csv").drop(['Unnamed: 0'], axis=1)
    ...
    # Strength prediction
    nrmse_one_2_1, avg_one_nrmse, avg_one_r2, avg_multi_nrmse, avg_multi_r2 = compareTwoVsOneEffect(one_way_train,
                                                                                                    y_train_one_way,
                                                                                                    one_way_test,
                                                                                                    y_test_one_way,
                                                                                                    X_train, X_test,
