...
class kNeighbors_regressor():
    ...
    def train(self):
        ...
        pickle.dump(self.model, open('KNeighbors_strength.sav', 'wb'))
    ...
    def hyperParameterTuning(self):
        ...
        kn = RandomizedSearchCV(estimator=self.model, scoring='neg_mean_squared_error', param_distributions=random_grid,
                                n_iter=500, cv=5,
                                verbose=2, n_jobs=-1)
        # fit the best model
...
class random_forest_regressor():
    ...
    def hyperParameterTuning(self):
        ...
        # find best model out of all range
        rf_random = RandomizedSearchCV(scoring="neg_mean_squared_error", estimator=self.model,
...
class xgb():
    ...
    def train(self):
        ...
        """
        self.hyperParameterTuning()
        pickle.dump(self.model, open('strength_model.sav', 'wb'))
    ...
    def hyperParameterTuning(self):
        ...
        # find best model out of all range
        xgb = RandomizedSearchCV(scoring="neg_mean_squared_error", estimator=XGR,
                                 param_distributions=random_grid,
                                 n_iter=500, cv=5,
                                 verbose=2, n_jobs=-1)
        # fit the best model
...
def plot_shap(XGR, X_test):
    ...
    """
    shap.initjs()
    explainer = shap.Explainer(XGR)
    shap_values = explainer(X_test)
    shap.plots.waterfall(shap_values[100], max_display=10, show=False)
